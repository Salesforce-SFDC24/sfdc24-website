...
class DecomposedPermissionSetTransformer extends baseMetadataTransformer_1.BaseMetadataTransformer {
    ...
    async toSourceFormat({ component, mergeWith }) {
        ...
        const writeInfoForParent = mergeWith
            ? [
                {
                    output: (0, node_path_1.join)((0, ts_types_1.ensureString)(mergeWith.content), `${component.name}.${(0, ts_types_1.ensureString)(component.type.suffix)}${constants_1.META_XML_SUFFIX}`),
...
const getDefaultOutput = (component) => {
    ...
    const output = (0, node_path_1.join)(parent?.type.strategies?.decomposition ? type.directoryName : '', `${childName ?? baseName}.${(0, ts_types_1.ensureString)(component.type.suffix)}${constants_1.META_XML_SUFFIX}`);
        ...
        nameWriteInfoMap.forEach((info) => {
            ...
            writeInfos.push({
                ...
                output: !info[0].mergeWith
                    ...
                    : (0, node_path_1.join)((0, ts_types_1.ensureString)(info[0].mergeWith.content), childInfo.type.directoryName, `${info[0].entryName}.${(0, ts_types_1.ensureString)(childInfo.type.suffix)}${constants_1.META_XML_SUFFIX}`),
            });
            return;
        });
    });
...
const toInfoContainer = (mergeWith) => (parent) => (childType) => (tagValue) => {
    ...
    const entryName = childType.directoryName
        ? (tagEntry.at(0)?.[childType.uniqueIdElement]).split('.')[0]
        : parent.name;
    return {
