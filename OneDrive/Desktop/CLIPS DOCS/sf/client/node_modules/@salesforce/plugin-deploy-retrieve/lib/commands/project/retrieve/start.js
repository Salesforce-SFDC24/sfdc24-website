...
import { RetrieveResult, ComponentSetBuilder, ComponentSet, RegistryAccess, RequestStatus, } from '@salesforce/source-deploy-retrieve';
...
export default class RetrieveMetadata extends SfCommand {
    ...
    static aliases = ['retrieve:metadata'];
    ...
    static configurationVariablesSection = toHelpSection('CONFIGURATION VARIABLES', OrgConfigProperties.TARGET_ORG, OrgConfigProperties.ORG_API_VERSION);
    ...
    async run() {
        ...
        this.retrieveResult = new RetrieveResult({}, componentSetFromNonDeletes);
        if (componentSetFromNonDeletes.size !== 0 || retrieveOpts.packageOptions?.length) {
            Lifecycle.getInstance().on('apiVersionRetrieve', async (apiData) => Promise.resolve(this.ms?.updateData({ apiData })));
        ...
        const formatter = flags['target-metadata-dir']
            ...
            : new RetrieveResultFormatter(new Ux({ jsonEnabled: this.jsonEnabled() }), this.retrieveResult, flags['package-name'], fileResponsesFromDelete);
        if (!this.jsonEnabled()) {
            ...
            if (!this.retrieveResult.response.status ||
                ...
                throw new SfError(getString(this.retrieveResult.response, 'errorMessage', this.retrieveResult.response.status), getString(this.retrieveResult.response, 'errorStatusCode', 'unknown'));
            }
        }
    ...
    async moveResultsForRetrieveTargetDir(targetDir, resolvedTargetDir) {
        async function mv(src) {
            ...
            await promisesQueue(files, async (file) => {
...
const buildRetrieveAndDeleteTargets = async (flags, format) => {
    ...
    if (isChanges) {
        if (!(await flags['target-org'].supportsSourceTracking())) {
            throw new SfError(messages.getMessage('noSourceTracking'), 'noSourceTracking', messages.getMessages('noSourceTracking.actions'));
        }
        ...
        return {
            componentSetFromNonDeletes: await ComponentSetBuilder.build({
                sourceapiversion: (await (await getOptionalProject())?.resolveProjectConfig())?.sourceApiVersion,
